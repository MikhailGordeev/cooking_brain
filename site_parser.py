...
while get_page:
    ...
    all = soup.findAll("div", {"class": "tile-list__horizontal-tile \
        horizontal-tile js-portions-count-parent js-bookmark__obj"})
    for i in all:
        try:
            recipe_id = i.find('div', {'class': 'bookmark js-tooltip \
                js-bookmark__label '}).attrs['data-id']
            title = i.find('div', {'class': 'lazy-load-container \
                js-lazy-loading'}).attrs['data-title']
            data_href = i.find('div', {'class': 'horizontal-tile__item-link \
                js-click-link '}).attrs['data-href']
            image_href = i.find('div', {'class': 'lazy-load-container \
                js-lazy-loading'}).attrs['data-src']

            tags = i.find('ul', {'class': 'breadcrumbs'})
            tags = ((tags.text).strip()).split('\n')

            ingridients_p = i.findAll('p', {'class': 'ingredients-list__content-item \
                content-item js-cart-ingredients'})
            ingridients = []
            for p in ingridients_p:
                ingridient = p.find('span', {'class': 'js-tooltip \
                    js-tooltip-ingredient'})
